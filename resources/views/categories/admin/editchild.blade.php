...
<h1 class="page-header">Редактирование подкатегории</h1>

<div class="row">
	<div class="col-md-8">
		<form class="form-horizontal" method="POST" action="/admin/categories/{{ $category->id }}/updatechild">
			...
			<div class="form-group">
		    	...
		    	<div class="col-sm-10">
		      		<input type="text" class="form-control" id="inputCategory" value="{{ $parent->name }}" disabled="true">
		    	</div>
		  	</div>
		    	...
		    	<div class="col-sm-4">
		      		<select id="inputStatus" class="form-control" name="status">
						@foreach (App\Category::statusesDropdown() as $value => $label)
							<option value="{{ $value }}" @if ($value == $category->status) selected="true" @endif>{{ $label }}</option>
						@endforeach
		      		</select>
		    	</div>
		  	</div>
